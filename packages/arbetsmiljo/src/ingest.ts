#!/usr/bin/env tsx
/**
 * Ingestion script for Arbetsmiljoverket (AV) regulations.
 *
 * Fetches the AV regulation index, scrapes each regulation page,
...
// ---------------------------------------------------------------------------
// Index page parsing — extract links to individual AFS regulations
// ---------------------------------------------------------------------------
...
function parseIndexPage(html: string): RegulationLink[] {
  ...
  // Match links containing AFS regulation references
  ...
  while ((match = linkRegex.exec(html)) !== null) {
    ...
    // Look for AFS YYYY:N pattern in the link text or URL
    const afsMatch = text.match(/AFS\s+(\d{4}:\d+)/i);
    if (!afsMatch) continue;

    const number = afsMatch[1];
    const id = `AFS-${number.replace(':', '-')}`;
    ...
    // Extract title (text before or after the AFS reference)
    const titleMatch = text.match(/AFS\s+\d{4}:\d+[,\s]*[-–—]\s*(.+)/i);
...
async function main(): Promise<void> {
  ...
  // Load previous hashes for diff detection
  const prevHashes = loadHashes();
  const newHashes: Record<string, string> = {};

  // Fetch all regulation pages
