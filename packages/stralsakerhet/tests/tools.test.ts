...
describe('stralsakerhet tool handlers', () => {
  ...
  it('about returns correct agency and gazette', () => {
    ...
    expect(result.agency).toBe('SSM');
    expect(result.gazette).toBe('SSMFS');
    expect(result.server).toBe('swedish-radiation-protection-mcp');
